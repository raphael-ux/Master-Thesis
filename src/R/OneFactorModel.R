...
UniFilter_onefactor <- function(vPw, in_sample_data, out_sample_data = NULL, alphas = c(0.01, 0.05), forecast = FALSE) {
  ...
  for(t in 2:iT) {
    ...
    current_kt <- kt[t-1]
    kt[t] <- B * current_kt + A * sum(lambda)
...
UniGASOptimiser_FLZ_onefactor <- function(vPw, data, alphas = c(0.01,0.05)) {
    iK <-2*length(alphas)
    iT <- length(data)
    vY <- as.numeric(data)
    res <- UniFilter_onefactor(vPw = vPw , in_sample_data = data  , alphas = alphas)
...
UniFit_onefactor <- function(data, alphas = c(0.01,0.05), fn.optimizer = fn.nloptr_OF.optim   ) {
    vY <- data
    iT <- length(vY)
    iK <- 2*length(alphas)
    ...
    vPw <- c(0.01,0.98,vab)
    optimiser <- fn.optimizer(par0 = vPw,data = data,alphas = alphas ,FUN = UniGASOptimiser_FLZ_onefactor)
...
UniRoll_onefactor <- function(data, alphas = c(0.01), ForecastLength = 50L, Nstart = NULL, RefitEvery = 2L, RefitWindow = c("moving","recursive"), cluster = NULL, Compute.SE = FALSE){
    vY <- data
    iT <- length(vY)
    iK <- 2*length(alphas)
  ...
  if (is.null(cluster)) {
    lFits <- lapply(seq_along(lData), function(i) {message("Running fit ", i, " of ", length(lData));UniFit_onefactor(lData[[i]], alphas = alphas)})
  } else {
    clusterEvalQ(cluster, {library(GAS);library(nloptr)})
    clusterExport(cluster, c("UniFit_onefactor", "fn.nloptr_OF.optim","UniGASOptimiser_FLZ_onefactor","UniFilter_onefactor"))
    lFits <- parLapply(cluster,lData,UniFit_onefactor,alphas = alphas)
  }
...
fn.nloptr_OF.optim <- function(par0, data, alphas, FUN) {
  ...
  lower <- c(1e-6, 0.5, rep(-Inf, 2*length(alphas)))
  upper <- c(0.01, 0.99, rep(Inf, 2*length(alphas)))
