...
library(dplyr)
library(knitr)
source("HybridModel.r")
source("OneFactorModel.R")
library(rugarch)
library(data.table)
library(parallel)
library(esback)
...
luGASRoll_norm <- list() #GAS-norm
luGASRoll_HY <- list() #HY
luGASRoll_OF <- list() #OF
########################cluster#######################################################################
...
for (i in 1:iN) {
  ...
  luGASRoll_norm[[i]] <- UniGASRoll(data = data_cl, GASSpec = uGASSpec_norm, ForecastLength = H,
                                    RefitEvery = RefitEvery, RefitWindow = RefitWindow , cluster = cluster )
  ...
  for (dAlpha in vAlpha){
      luGASRoll_HY[[i]][[paste(dAlpha)]] <- UniRoll_hybridmodel(data = data_cl , alphas = c(dAlpha)  , ForecastLength = H , Nstart = NULL ,
                                   RefitEvery =RefitEvery,RefitWindow = RefitWindow , cluster = cluster)
      luGASRoll_OF[[i]][[paste(dAlpha)]] <- UniRoll_onefactor(data = data_cl , alphas = c(dAlpha) , ForecastLength = H, Nstart = NULL,
                                    RefitEvery = RefitEvery , RefitWindow = RefitWindow , cluster = cluster)
  }
}
  ###############################################################################################################
...
names(luGASRoll_HY) <- vAsset
names(luGASRoll_OF) <- vAsset
...
lVaR <- lapply( vAsset, function(sAsset) {
  ...
  for (sDist in vDist) {
    ...
    for (iAlpha in seq_along(vAlpha)) {  
      ...
      if(sDist %in% c("HY", "OF")) {
        mOut[, sDist, paste(dAlpha)] <- obj_GAS[[sAsset]][[paste(dAlpha)]]$Forecasts[[1]]
      }else{
...
lES <- lapply( vAsset, function(sAsset) {
  ...
  for (sDist in vDist) {
    ...
    for (iAlpha in seq_along(vAlpha)) { 
      ...
      if(sDist %in% c("HY", "OF")) {
        mOut[, sDist, paste(dAlpha)] <- obj_GAS[[sAsset]][[paste(dAlpha)]]$Forecasts[[2]]
      }else{
...
sDir_Tables <- "../results/VaR_ES_Backtesting/GAS_Tables"  
