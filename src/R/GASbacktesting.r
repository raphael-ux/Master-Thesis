...
rolling_portfolio <- function(initial_date,dates,H,F,type = c("volatility", "ES" , "VaR") ,cluster = NULL){
    ...
    for(i in 1:(length(cDates)-1)){
        ...
        for(asset in available){
            ...
            #GAS part #################################################################
            fit <- UniGASFit(GASSpec = uGASSpec_std, data = log_data_asset)
            forecast <- UniGASFor(uGASFit = fit, H = H, Roll = FALSE, B = 10000L, Bands = c(0.1,0.15,0.85,0.9), ReturnDraws = TRUE)
            if(anyNA(forecast@Forecast$PointForecast)){
              sorting[[paste(current_date)]][[asset]] <- NA
              next
            }

            if(anyNA(forecast@Draws)){
              mDraws <- forecast@Draws
              good_paths <- apply(mDraws, 1, function(row) all(is.finite(row)))
              forecast@Draws <- mDraws[good_paths, , drop = FALSE]
            }

            ES[[paste(current_date)]][[as.character(asset)]] <- min(ES(forecast,0.01))
            VaR[[paste(current_date)]][[as.character(asset)]] <- min(quantile(forecast,0.01))
            vola[[paste(current_date)]][[as.character(asset)]] <- mean(as.numeric(forecast@Forecast$PointForecast[,"scale"]))
            sorting[[paste(current_date)]][[as.character(asset)]] <- vola[[paste(current_date)]][[as.character(asset)]]
        }
    }
...
cut_date = as.Date("2000-01-07")
idx_cut = match(cut_date,cDates)
