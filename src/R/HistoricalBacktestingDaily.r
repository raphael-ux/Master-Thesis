    ...
    for(i in 1:length(dates_)){
        ...
        if(current_date %in% cDates){
            ...
            for(asset in available){
                ...
                sorting[[as.character(asset)]] <- sd(log_data_asset)
                vola[[paste(current_date)]][[as.character(asset)]] <- sd(log_data_asset)
                VaR[[paste(current_date)]][[as.character(asset)]] <- as.numeric(quantile(log_data_asset,0.01))
                ES[[paste(current_date)]][[as.character(asset)]] <-  as.numeric(mean(log_data_asset[log_data_asset <= VaR[[paste(current_date)]][[as.character(asset)]]]))
            ...
            for(asset in available__){
                ...
                sorting[[as.character(asset)]] <- sd(path[[as.character(asset)]])
                vola[[paste(current_date)]][[as.character(asset)]] <- sd(path[[as.character(asset)]])
                VaR[[paste(current_date)]][[as.character(asset)]] <- as.numeric(quantile(path[[as.character(asset)]],0.01))
                ES[[paste(current_date)]][[as.character(asset)]] <-  as.numeric(mean(path[[as.character(asset)]][path[[as.character(asset)]] <= VaR[[paste(current_date)]][[as.character(asset)]]]))
            }
        }
